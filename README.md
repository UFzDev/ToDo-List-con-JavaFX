...
### Error: `Location is not set` al cargar FXML

Causa comun:

- Nombre/ruta de FXML cambiada por refactor y no actualizada en `NavigationUtil`.

Revision:

- Verificar nombre exacto del archivo en `resources/view`.
- Verificar llamadas `renderView(..., "archivo.fxml", ...)`.

### Error de modulos Google Storage duplicados

Mensaje tipico:

- `the unnamed module reads package ... from both gapic... and proto...`

Accion:

- Excluir dependencia duplicada conflictiva en Gradle (si aplica a tu arbol real).

### Warning SLF4J sin provider
...
- Con `runtimeOnly("org.slf4j:slf4j-simple:2.0.16")` suele quedar resuelto.

### Error de coercion en FXML (`Insets` to `Node`)

Causa comun:

- Nodo colocado en una propiedad equivocada del layout.

Accion:

- Revisar jerarquia y propiedades validas del contenedor (`VBox`, `HBox`, `BorderPane`, etc).

## 14. Roadmap sugerido

- Completar logica de exportacion real en reportes (PDF/Excel).
- Agregar pruebas unitarias a servicios y utilidades de estadisticas.
- Agregar pruebas de integracion para DAOs con entorno de test.
- Endurecer manejo de errores y logs estructurados.
- Separar configuracion por perfiles (dev/prod).
...
Si vas a extender el proyecto, empieza por `TaskService`, `UserSessionUtil` y `NavigationUtil`: son los puntos de mayor impacto en comportamiento global.
